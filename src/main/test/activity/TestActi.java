...
//        获取流程相关的服务
        ...
        assert (object == null);
        List<Task> taskList = taskService.createTaskQuery().processInstanceId("90001").list();
//        List<Task> taskList = taskService.createTaskQuery().taskCandidateOrAssigned();
        ...
        for(Task t : taskList){
            Map map3 = taskService.getVariablesLocal(task.getId());
            System.out.println();
        }
        HistoryService historyService = processEngine.getHistoryService();
        List<HistoricTaskInstance> list2 = historyService.createHistoricTaskInstanceQuery().processInstanceId("90001").list();

        Map map4;
        for(HistoricTaskInstance historicTaskInstance : list2){
            System.out.println(historicTaskInstance.getId());
            map4 = historicTaskInstance.getProcessVariables();
            System.out.println();
        }

        List<HistoricVariableInstance> list3 = historyService.createHistoricVariableInstanceQuery().processInstanceId("90001").list();
        for(HistoricVariableInstance historicTaskInstance : list3){
            System.out.println(historicTaskInstance.getId());
            String str = historicTaskInstance.getVariableName();
            System.out.println();
        }
